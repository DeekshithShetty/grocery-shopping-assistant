...
Mat labelPreprocessingFromMatlab(Mat image){
    Mat tophat,element,grey;

    element = cv::getStructuringElement(MORPH_ELLIPSE, cv::Size(9,9));
    cv::morphologyEx(image, tophat, CV_MOP_TOPHAT, element);
    if (tophat.channels() == 3){
        cvtColor(image, grey, CV_BGR2GRAY);
    } else {
        grey = image;
    }
    adaptiveThreshold(grey, grey, 255, CV_ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY, 15, -2);
    imshow("preprocess_grey", grey);

    Mat lineStructure = getStructuringElement(MORPH_RECT, Size( 10,0));
    erode(tophat, grey, lineStructure, Point(-1, -1));
    dilate(tophat, grey, lineStructure, Point(-1, -1));
    adaptiveThreshold(grey, grey, 255, CV_ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY, 15, -2);
    imshow("preprocess_grey2", grey);
...
Mat removeLines(Mat image){
    ...
    Mat linek = Mat::zeros(Size(11,11),CV_8UC1);
    linek.row(5) = 255;  
    cv::morphologyEx(grey, x, CV_MOP_OPEN, linek);
    grey = grey - x; 

    imshow("lines removed", grey);

    return grey; 
}

Mat removeLines2(Mat image){

    Mat grey, dst;
    vector<Vec4i> lines;

    Canny(image, grey, 50, 200, 3);
    if (grey.channels() == 3){
    ...
    HoughLinesP(grey, lines, 80, CV_PI/2, CV_THRESH_OTSU|CV_THRESH_BINARY, 25, 7);
    for(size_t i = 0; i < lines.size(); i++){

      Vec4i l = lines[i];
      line(grey, Point(l[0], l[1]), Point(l[2], l[3]), Scalar(255,255,255), 8, 8);
