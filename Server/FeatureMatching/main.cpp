...
int main( int argc, char** argv ){
  ...
  argvStrings[2] = string(argv[2]); //image that is to be checked 
  ...
  for(i = 3; i < argc; i++){
    img_1 = imread( argvStrings[2].c_str(), CV_LOAD_IMAGE_GRAYSCALE );
...
bool checkSimilarityInImages(float image1, float image2, string imageName1, string imageName2){
  if(image1 > image2){
    ...
    if(image2 > (image1 * 0.75)){
      ...
      cout << "--- The two images - " << imageName1 << " and " << imageName2 << " are similar ---" << endl;
      return true;
    } else {

      cout << "--- The two images - " << imageName1 << " and " << imageName2 << " are  not similar ---" << endl;
    }
